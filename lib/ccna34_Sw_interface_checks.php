...
function check_Sw_interface_config($interface_text, $interface_details, $expected_details)
{
  ...
  {
    LogError("minor", "Interfaces not configured for the following interfaces:", $long_description);
    return;
  }

  // Check if the interface is shutdown
  // Major error for trunk port, minor for others
  if ((isset($interface_details->shutdown)) && ($interface_details->shutdown))
  {
	if ($expected_details['port_mode']=="trunk")
			LogError("major", "The following trunk interface (s) were shutdown:", $long_description);
	else 
			LogError("minor", "The following interface(s) were shutdown:", $long_description);
  }	
  ...
  if (isset($interface_details->address))
  {
	if (isset($expected_details['address']))
	{
		...
		//if ($interface_details->host_id != $host_id) - error generated - to fix if want to use this option.
		if($interface_details->address != (long2ip(ip2long($network) + $host_id)))
		{
  ...
  
  {
	  ...
	  if ($security_enable == "on")
	  {
		if ($interface_details->security->security_enable != TRUE)
			LogError("minor", "Did not enable switchport security for the following interface(s):", "$long_description ");
...
function check_Sw_interface_unconfigured($interface_text, $interface_details)
{
  ...
  // Check an IP address has been configured
  if (isset($interface_details->address))
    LogError("minor", "You have configured the following interfaces when you should not have", $interface_text);
	return;
  //else
...
function check_Sw_link_config($description, $interface_details1, $interface_details2, $serial_link)
{
  ...
  if ($interface_details1->address == $interface_details2->address)
    LogError("minor", "IP Addresses on either side of network link are the same", "$description: Both set to " . $interface_details1->address);  

  // Serial link, check for clock rate
  if (($serial_link) && (is_null($interface_details1->clock)) && (is_null($interface_details2->clock)))
    LogError("minor", "Clock rate not set on DCE interface of serial Link", $description);  
}
